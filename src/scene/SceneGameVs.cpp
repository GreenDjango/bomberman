...
void SceneGameVs::init(IrrlichtDevice *device, GameStats *gameS, CameraManager *cameraM, SoundManager *soundM)
{
    _device = device;
...
void SceneGameVs::start(u32 startTime)
{
...
void SceneGameVs::draw()
{
    ...
    _fonts[GUI::STATS]->draw(msToString(_gameS->remainingTime), _gui[GUI::TIME]->getAbsoluteClippingRect(), video::SColor(240,255,255,255), true, true);
     if (_isEnd) {
        ...
        core::stringw winner{L"Draw !"};
        if (_winnerId == 1)
            winner = L"PLayer 1 win !";
        else if (_winnerId == 2)
            winner = L"PLayer 2 win !";
        _fonts[GUI::END_PNL]->draw(winner, _gui[GUI::END_PNL]->getAbsoluteClippingRect(), video::SColor(240,255,255,255), true, true);
    }
    if (_showHelp) {
        _driver->draw2DRectangle(video::SColor(130, 0, 0, 0), core::rect<s32>(0,0,1920,1080));
...
void SceneGameVs::clean()
{
    for (auto el : _gui)
        el.second->setVisible(false);
}

bool SceneGameVs::OnTimer(u32, SceneID&)
{
    if (_isEnd)
        return false;
    if (_gameS->players[0].life < 0 ||  _gameS->players[1].life < 0) {
        _soundM->playTheme(SoundManager::Theme::MENU);
        _soundM->audienceCheer();
        _soundM->audienceClap();
        _isEnd = true;
        _gameS->isFinish = true;
        if (_gameS->players[0].life < 0)
            _winnerId = 2;
        else
            _winnerId = 1;
        ...
        _isEnd = true;
        _gameS->isFinish = true;
        if (_gameS->players[0].life < _gameS->players[1].life)
            _winnerId = 2;
        else if (_gameS->players[0].life > _gameS->players[1].life)
            _winnerId = 1;
        _gui[GUI::END_PNL]->setVisible(true);
    }
    return false;
...
bool SceneGameVs::OnEvent(const SEvent& event, SceneID&)
{
